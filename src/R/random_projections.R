...
option_list <- list( 
  ...
  make_option(c("-o", "--overwrite"), action="store_true", default=FALSE, 
              help="Overwrite output file if already present [default false]"), 
  make_option(c("-m", "--matrix"), action="store_true", default=FALSE, 
              help="Save matrix (in output_dir) used for random projections [default false]"), 
  make_option(c("-b", "--backup"), action="store_true", default=FALSE, 
              help="Make backup (in output_dir) of kmers in csv format [default false]")
  )


args <- parse_args(OptionParser(usage = "%script [options] kmers_in dimensions_out input_Rdata_file output_dir", 
...
object_name <- tryCatch(
  ...
  finally={message("Loaded .RData input file")}
)
if(length(object_name) > 1) stop("Only one object per .RData file is supported")
...
X <- matrix(0, nrow = n_to_process, ncol = kmer_perms)
...
if(opt$backup) {
  ...
  if(file.exists(backup_file) & !opt$overwrite) message("Skipping backup as file is already present. Use --overwrite to force overwrite. ")
  else if(file.exists(backup_file)) {
    message(paste("Overwriting kmer backup to", backup_file))
    write.csv(cbind(names(data), X), backup_file, row.names = F)
  } else {
...
R <- matrix(rnorm(kmer_perms * dimensions_out), nrow=dimensions_out, ncol=kmer_perms)

if(opt$matrix) {
  matrix_backup <- file.path(output_dir, paste0(kmers, "_kmer_random_matrix.csv"))
  if(file.exists(matrix_backup) & !opt$overwrite) message("Random matrix backup - file already exists, skipping. Use --overwrite to force overwrite")
  else if(file.exists(matrix_backup)) {
    message(paste("Overwriting random matrix backup to", matrix_backup))
    write.csv(R, matrix_backup, row.names = F)
  }
  else {
...
X_rp <- R %*% t(X)
X_rp <- (1 / sqrt(length(head(data, n_to_process)))) * X_rp

X_rp <- t(X_rp)
X_rp <- cbind(names(data), X_rp)

message("Saving output..")
output_file <- file.path(output_dir, paste0(kmers, "_kmer_random_projections.csv"))
if(file.exists(output_file) & !opt$overwrite) { 
  ...
  write.csv(X_rp, output_file, row.names = F)
